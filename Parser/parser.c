...
#define optional_name_type 1143
...
static expr_ty optional_name_rule(Parser *p);
...
star_atom_rule(Parser *p)
{
    ...
    { // optional_name
        ...
        D(fprintf(stderr, "%*c> star_atom[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "optional_name"));
        expr_ty optional_name_var;
        if (
            (optional_name_var = optional_name_rule(p))  // optional_name
        )
        {
            D(fprintf(stderr, "%*c+ star_atom[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "optional_name"));
            _res = optional_name_var;
            goto done;
        }
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s star_atom[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "optional_name"));
    }
    { // '(' star_target ')'
        if (p->error_indicator) {
            D(p->level--);
...
// t_atom: optional_name | '(' target ')' | '(' targets? ')' | '[' targets? ']'
static expr_ty
...
optional_name_rule(Parser *p)
{
    ...
    { // NAME
        ...
        D(fprintf(stderr, "%*c> optional_name[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "NAME"));
        expr_ty a;
        if (
            (a = _PyPegen_name_token(p))  // NAME
        )
        {
            D(fprintf(stderr, "%*c+ optional_name[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "NAME"));
            _res = _PyPegen_set_expr_context ( p , a , Store );
            if (_res == NULL && PyErr_Occurred()) {
                p->error_indicator = 1;
                D(p->level--);
        ...
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s optional_name[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "NAME"));
    }
    { // '?'
        if (p->error_indicator) {
        ...
        )
        {
            ...
            int _end_col_offset = _token->end_col_offset;
            UNUSED(_end_col_offset); // Only used by EXTRA macro
            _res = CHECK_VERSION ( 10 , "unused assignment syntax is" , _Py_Name ( NULL , Store , EXTRA ) );
            if (_res == NULL && PyErr_Occurred()) {
                p->error_indicator = 1;
                D(p->level--);
                return NULL;
